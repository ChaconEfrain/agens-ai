...
export default function PricingCard({Icon, description, includes, label, plan, price}: Props) {
  return (
    <Card className={cn("min-h-[500px] border-2", {
      'border-primary relative md:scale-105 shadow-lg overflow-hidden': plan === 'Basic',
      'border-muted': plan !== 'Basic'
    })}>
      {plan === 'Basic' && (
        <div className="absolute top-6 -right-24 rotate-45 w-64">
          <Badge className="bg-primary text-primary-foreground w-full">Most Popular</Badge>
        </div>
      )}
      <CardHeader>
        ...
        <CardTitle className="text-2xl"><span>{label}</span></CardTitle>
      ...
      <CardFooter className="mt-auto flex flex-col">
        {plan === 'Pro' && <p className='self-start text-sm text-muted-foreground mb-4'>*$1 per extra 1,000 messages</p>}
